...
class Transformer_encoder_l(Module):
    ...
    def __init__(self, d_model: int = 512, nhead: int = 8, num_encoder_layers: int = 6, dim_feedforward: int = 384, dropout: float = 0.1,
        ...
        super(Transformer_encoder_l, self).__init__()
...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: Tensor, srcc: Tensor, src_mask: Optional[Tensor] = None,
        ...
        src = self.modulation(srcs1.reshape(b, c, int(16), int(8)),
                              srcc.contiguous().reshape(b, c, int(16), int(8))).reshape(b, c, -1).permute(2, 0, 1)
